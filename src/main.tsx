...
function HourglassTimer({total,setTotal,remaining,running,paused,finished,onToggle,onReset,onSetRemaining}:{total:string;setTotal:(v:string)=>void;remaining:number;running:boolean;paused:boolean;finished:boolean;onToggle:()=>void;onReset:()=>void;onSetRemaining:(seconds:number)=>void}){
 const seconds=parseDuration(total)||1,[kick,setKick]=useState(0),[resetBreaking,setResetBreaking]=useState(false),[resetRetracting,setResetRetracting]=useState(false),resetBreakTimer=useRef<ReturnType<typeof setTimeout>|undefined>(undefined);
 useEffect(()=>()=>clearTimeout(resetBreakTimer.current),[]);
 useEffect(()=>{if(paused)setResetRetracting(false)},[paused]);
 const react=()=>setKick(value=>value+1);
 const beginResetBreak=()=>{setResetBreaking(true);clearTimeout(resetBreakTimer.current);resetBreakTimer.current=setTimeout(()=>setResetBreaking(false),420)};
 return <section className={`timer ${running?'running':''} ${finished?'finished':''} ${paused?'paused':''}`} aria-label="Timer"><div className="timer-name">TIMER</div><RemainingReadout remaining={remaining} editable={running||paused||finished} breaking={resetBreaking} onSetRemaining={onSetRemaining}/><div className={`hourglass ${kick?kick%2?'wobble-right':'wobble-left':''}`} onPointerDown={react}><svg viewBox="0 0 130 300" preserveAspectRatio="none" aria-hidden="true"><path d="M12 12L116 15M15 20Q12 85 57 139Q67 148 56 165Q11 218 16 281M114 21Q117 78 72 138Q59 149 74 167Q115 220 117 280M11 287L121 290M20 6L112 9M18 296L115 296" fill="none" stroke="#050505" strokeWidth="5" strokeLinecap="square"/><SandGrains seconds={seconds} remaining={remaining} kick={kick} paused={paused}/></svg><button className="timer-control hard" onPointerDown={paused?e=>{e.stopPropagation();setResetRetracting(true)}:undefined} onClick={onToggle}>{running?'PAUSE':paused?'CONTINUE':'START'}</button><TimerReset open={(paused||finished)&&!resetRetracting} onResetStart={beginResetBreak} onReset={onReset}/></div><TotalCard total={total} setTotal={setTotal} locked={running||paused||finished}/>{finished&&<span className="done" role="status">TIME'S UP</span>}</section>
}
...
function TimerReset({open,onResetStart,onReset}:{open:boolean;onResetStart:()=>void;onReset:()=>void}){
 return <button className={`timer-reset ${open?'is-open':''}`} disabled={!open} aria-hidden={!open} tabIndex={open?0:-1} onPointerDown={onResetStart} onClick={e=>{onResetStart();e.currentTarget.parentElement?.querySelector<HTMLButtonElement>('.timer-control')?.focus();onReset()}} aria-label="Reset timer">RESET</button>;
}
...
function handsetOrientationCorrection(){
